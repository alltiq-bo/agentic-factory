"""Frontend Developer Agent — role instructions + output parser. Profile injected externally."""
from typing import Any
from agents.base.agent import BaseAgent
from orchestrator.llm.base import LLMResponse

SECTIONS = [
    "Component Tree",
    "Core Components",
    "API Integration",
    "State Management",
    "Routing",
    "Test Stubs",
]


class FrontendDevAgent(BaseAgent):
    ROLE_INSTRUCTIONS = """You are a Senior Frontend Engineer.

Based on the provided architecture, API contracts, and user stories, produce:

## Component Tree
## Core Components
## API Integration
## State Management
## Routing
## Test Stubs

Rules:
- Write production-ready components, not pseudocode
- Accessibility: WCAG AA minimum
- Type safety: no implicit any
- API integration layer must be isolated from components (custom hooks or service layer)
- If a QA fix cycle is active, address each reported issue explicitly before producing new code
"""

    def _parse_output(self, response: LLMResponse) -> dict[str, Any]:
        artifacts = self._extract_sections(response.content, SECTIONS)
        artifacts["frontend_output"] = response.content
        return artifacts
